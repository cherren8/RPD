# Figures for AGP Antibiotics, AGP C. diff, and Ghana Breast Health Analyses
...
AIC(model.ab.rpd) - AIC(model.ab.bcd)
...
roc_obj.ab.bcd <- roc(ab.data$case.control.binary.abx, preds.ab.bcd)
...
legend("topleft", legend = c("RPD", "BCD"), col = c("blue", "red"), lwd = 2, cex = 1.2, bty = "n")

# Calculate DeLong's test
roc.test(roc_obj.ab.rpd, roc_obj.ab.bcd, method = "delong")
...
AIC(model.cd.rpd) - AIC(model.cd.bcd)
...
roc_obj.cd.bcd <- roc(cd.data$case.control.binary.cd, preds.cd.bcd)
...
# Calculate DeLong's test
roc.test(roc_obj.cd.rpd, roc_obj.cd.bcd, method = "delong")
...
AIC(model.br.rpd) - AIC(model.br.bcd)
...
# 3. Create and plot the ROC curve
roc_obj.br.rpd <- roc(br.data$case.control.binary.gbh, preds.br.rpd)
plot(roc_obj.br.rpd, print.auc = TRUE, col = "blue", main = "ROC for Ghana Breast Health",
     print.auc.x = .4,
     print.auc.y = .5,
     print.auc.cex = 1.5)

roc_obj.br.bcd <- roc(br.data$case.control.binary.gbh, preds.br.bcd)
plot(roc_obj.br.bcd, print.auc = TRUE, col = "red", main = "ROC for Breast BCD",
     add = T,
     print.auc.x = .4,
     print.auc.y = .4,
     print.auc.cex = 1.5)
...
# Calculate DeLong's test
roc.test(roc_obj.br.rpd, roc_obj.br.bcd, method = "delong")

## Do comparisons of how mong algorithms take to run

# # AGP antibiotics
# system.time({
#   calc.rpd(ab.ref, ab.test, ref.pers = 0.7)
# })
#
# system.time({
#   as.matrix(vegdist(rbind(ab.ref.mean, ab.test)))[-1, 1]
# })
#
# # ACP C. diff
# system.time({
#   calc.rpd(cd.ref, cd.test, ref.pers = 0.6)
# })
#
# system.time({
#   as.matrix(vegdist(rbind(cd.ref.mean, cd.test)))[-1, 1]
# })
#
# # GBH
# system.time({
#   calc.rpd(ref = b.num[ref, ], test = b.num[c(case, control), ], ref.pers = ref.threshold)
# })
#
# system.time({
#   as.matrix(vegdist(rbind(bc.ref.mean.gbh, b.num.bc)))[-1, 1]
# })
